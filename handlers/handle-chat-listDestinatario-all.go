...
func ChatListAllDestinatarioHandler(w http.ResponseWriter, r *http.Request) {
	fmt.Println("Got a /chat/get/destinatario handle from ", r.RemoteAddr)
	...
	case "GET":
		...
		//Eseguo la query
		queyRes, queyErr := db.Query("SELECT * FROM `chat` WHERE `IDUtenteDestinatario`='" + id + "'")
		if queyErr != nil {
			fmt.Println(queyErr)
			w.Write([]byte(`{"message": "error"}`))
			return

...
func UserHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "GET":
		...
		//Execute query
		queyRes, queyErr := db.Query("SELECT * FROM utente WHERE Username='" + usrName + "'")
		if queyErr != nil {
			fmt.Println(queyErr)
			w.Write([]byte(`{"message": "error"}`))
			return
	...
	case "PUT":
		...
		/*
			toRegisterUser.Sesso = r.FormValue("sesso")
			toRegisterUser.DataNascita = r.FormValue("dataNascita") */
